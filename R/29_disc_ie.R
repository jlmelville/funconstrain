...
#'   functions `m = n`.
#' - Minima: `f = 0` for every valid `n` (at the same location as
#'   [disc_bv()]). The returned `fmin` is this dimension-independent value,
#'   while the returned `xmin` is the `n = 5` example.
#'
    ...
    he = function(x) {
      ...
      for (i in 1:n) {
        ...
        for (j in 1:i) {
          ...
          h[j, j] <- h[j, j] +
            2.0 * t * (3.0 * d1 * (1.0 - t1) * t2 * (x[j] + t2 + 1.0))
        }
        ...
        if (i < n) {
          for (j in (i + 1):n) {
            ...
            h[j, j] <- h[j, j] +
              2.0 * t * (3.0 * d1 * t1 * (1.0 - t2) * (x[j] + t2 + 1.0))
          }
        }
        #          cat("new k loop\n")
        for (k in 1:n) {
