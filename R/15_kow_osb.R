    ...
    he = function(par) {
      ...
      for (i in 1:m) {
        ...
        if (t1 != 0.0) {
          ...
          h[1, 3] <- h[1, 3] +
            2.0 * s1 * u[i]^2 / t1^2 * (d1 - u[i] * s1 * x1 / t1)
          h[2, 3] <- h[2, 3] +
            2.0 * x1 * u[i]^2 / t1^2 * (d1 - x1 * u[i] * s1 / t1)
          h[3, 3] <- h[3, 3] +
            2.0 * u[i]^3 * x1 * s1 / t1^3 * (x1 * u[i] * s1 / t1 - 2.0 * d1)
          h[1, 4] <- h[1, 4] +
            2.0 * u[i] * s1 / t1^2 * (d1 - u[i] * s1 * x1 / t1)
          h[2, 4] <- h[2, 4] +
            2.0 * x1 * u[i] / t1^2 * (d1 - x1 * u[i] * s1 / t1)
          h[3, 4] <- h[3, 4] +
            2.0 * x1 * u[i]^2 * s1 / t1^3 * (x1 * u[i] * s1 / t1 - 2.0 * d1)
          h[4, 4] <- h[4, 4] +
