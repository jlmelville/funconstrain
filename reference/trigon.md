...
and `trigon()`. It is not part of the core return contract needed by
optimizers.
...
- Minima: `f = 0` at `rep(0, n)` for every valid `n`, but there are many
  others. The returned `fmin` is this dimension-independent value, while
  the returned `xmin` is the `n = 4` example.
...
It seems to be extremely difficult to reach the global minimum for any
`n` from the starting location using the typical gradient-based methods
